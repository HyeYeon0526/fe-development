...
export function test1() {
    axios.get('/students')
        // 서버의 http://localhost:3000/students URL을 get 요청한다 
...
export function test2() {
    axios.get('/students').then(response => {
        let students = response.data; return students[0].id; // 이전의 then 콜백 함수의 리턴 값이 }) 
    // 다음 then 콜백 함수의 파라미터로 전달된다. 
    
    .then(id => { 
        axios.get('/students/' + id)
    .then(response => { let student = response.data; console.log('test2', student);
 })
 })
    } 
...
export function test3(id) {
        axios.get('/students/' + id)
        .then(response => { let student = response.data; console.log('test3', student); }).catch(error => { // 에러가 발생하면, catch 콜백 함수가 호출된다. 
